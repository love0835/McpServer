...
TEST_ROOT = Path(tempfile.mkdtemp(prefix="codex_bridge_allowed_"))
STATE_ROOT = Path(tempfile.mkdtemp(prefix="codex_bridge_state_"))
FAKE_ENTRY = STATE_ROOT / "codex.js"
FAKE_ENTRY.write_text("// fake", encoding="utf-8")
os.environ["CODEX_NODE_EXE"] = sys.executable
os.environ["CODEX_JS_ENTRY"] = str(FAKE_ENTRY)
os.environ["CODEX_BRIDGE_ALLOWED_DIRS"] = str(TEST_ROOT)
os.environ["CODEX_BRIDGE_STATE_DIR"] = str(STATE_ROOT)
os.environ["CODEX_BRIDGE_ENABLE_ASK"] = "1"
os.environ["CODEX_BRIDGE_HOST"] = "127.0.0.1"
os.environ["CODEX_BRIDGE_PORT"] = "18998"
os.environ["CODEX_BRIDGE_MAX_CONCURRENT"] = "1"
...
def wait_done(job_id: str):
    ...
    while time.time() < deadline:
        payload = parse(server.get_codex_job(job_id))
        if payload["done"]:
            return payload
        time.sleep(0.05)
...
def main() -> int:
    ...
    try:
        attach = parse(server.codex_attach_prompt("large prompt body"))
